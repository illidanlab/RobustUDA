...
def main():
    ...
    parser.add_argument('--poison', type=int, default=0,
                        help='poison or not')
    parser.add_argument('--poison_num', type=int, default=20,
                        help='The number of poison samples.')
    ...
    if args.poison != 0:
        source_samples, source_labels = generate_image(source_path, args.poison_num)
        source_samples, source_labels = torch.Tensor(source_samples).to(
            args.device), torch.LongTensor(source_labels).to(args.device)
        print('finish poison! Poison data number is {}'.format(args.poison_num))
