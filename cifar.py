...
class MoMDataLoader():
    def __init__(self, training_data, batch_size):
        ...
        for x_i, y_i, p_i in training_data:
            data_set = SimpleDataSet(x_i, y_i, p_i)
...
def image_classification_test_loaded(b_size, dset, model, device='cpu'):
    with torch.no_grad():
        ...
        correct = 0
        pred_result = []
        for ii, (sample, target, p) in enumerate(dset):
            #print("test label", target)
            #print("test poison ind", p)
            #if ii == 0:
            #    imshow(data)
...
def vote(b_size, pred, dset, out_log_file, name):
    num = len(pred)
    correct = 0
    for i, (sample, target,_) in enumerate(dset):
        pred_i = []
        for j in range(num):
...
def train(config, source_samples, source_labels, target_samples, target_labels, test_samples, test_labels):
    ...
    if args.poison != 0:
        source_samples, source_labels, p_source = generate_image3(source_samples, source_labels, x, y, config["network"]["params"]["class_num"], args.poison_ratio)
    source_samples, source_labels, p_source = torch.Tensor(source_samples).to(
            config["device"]), torch.LongTensor(source_labels).to(config["device"]), torch.Tensor(p_source).to(
            config["device"])
    test_samples_poison, test_labels_poison, p_test_poison = generate_image3(test_samples, test_labels, x, y, config["network"]["params"]["class_num"], 1)
    test_samples_poison, test_labels_poison, p_test_poison = torch.Tensor(test_samples_poison).to(
        config["device"]), torch.LongTensor(test_labels_poison).to(config["device"]), torch.Tensor(p_test_poison).to(
        config["device"])
    test_samples, test_labels, p_test = torch.Tensor(test_samples).to(
        config["device"]), torch.LongTensor(test_labels).to(config["device"]), torch.zeros(test_samples.shape[0]).to(
        config["device"])
    ...
    for group_idx in range(args.block):
        if group_idx == args.block - 1:
            ...
            p_source_i = p_source[(n_size_t * group_idx):]
        else:
            ...
            p_source_i = p_source[(n_size_t * group_idx): n_size_t * (group_idx + 1)]
        p_target_i = torch.zeros((target_labels_i.shape[0]))
        training_source.append((source_samples_i, source_labels_i, p_source_i))
        training_target.append((target_samples_i, target_labels_i, p_target_i))
    ...
    dset_loaders["test"] = DataLoader(SimpleDataSet(test_samples, test_labels, p_test), batch_size=test_bs, \
                            shuffle=False, num_workers=4, drop_last=True)
    dset_loaders["test_poison"] = DataLoader(SimpleDataSet(test_samples_poison, test_labels_poison, p_test_poison), batch_size=test_bs, \
                            shuffle=False, num_workers=4, drop_last=True)
    ...
    for model_idx in range(args.block):
        ...
        print("len train source", len_train_source)
        ...
        for i in range(config["num_iterations"]):
            if i % config["test_interval"] == config["test_interval"] - 1:
                ...
                log_str = "  iter: {:05d}, sec: {:.0f}, class: {:.5f}, da: {:.5f}, precision: {:.5f}, success_rate: {:.5f}".format(
                    i, time.time() - start_time_test, classifier_loss_value, transfer_loss_value, temp_acc, test_poison_acc)
            ...
            inputs_source, label_source, p_source = iter_source.next()
            ...
            #print("p_ind", p_source)
            #imshow(inputs_source)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_iterations', type=int, default='2000', help="Number of batch updates")
    ...
    if args.s_dset != args.t_dset:
        ...
        test_samples, test_labels = torch.Tensor(d_source.test_X), torch.LongTensor(d_source.test_y)
        #print("source sample shape", source_samples.shape)
        print("source label", torch.unique(source_labels))
        #print("target sample shape", target_samples.shape)
        print("target label", torch.unique(target_labels))
        #print("test sample shape", test_samples.shape)
        print("test label", torch.unique(test_labels))
