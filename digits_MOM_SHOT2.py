...
def train_target(config,model,out_log_file, out_log_file_train, model_idx,target_samples, target_labels, test_samples, test_labels, test_samples_poison,test_labels_poison, source_samples, source_labels, args):
    ...
    for epoch in range(1, args.SHOT_epochs + 1):
        optimizer.zero_grad()
...
def main():
    ...
    parser.add_argument('--SHOT_epochs', type=int, default=70, metavar='N',
                        help='number of epochs to train (default: 70) for SHOT')
    ...
    print('Running {} on {} for {} epochs on task {}2{}'.format(
        args.method, args.device, args.epochs, args.s_dset, args.t_dset))
    ...
    for model_idx in range(args.block):
        ...
        pred_test, pred_test_poison, pred_source = train_target(config,model, out_log_file, out_log_file_train, model_idx, target_samples, target_labels, test_samples,
