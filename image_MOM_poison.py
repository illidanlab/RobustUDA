...
def train(config):
    ...
    if args.noise_type == 'poison':
        ...
        test_samples_poison, test_labels_poison = generate_image(test_samples, test_labels, x, y,
        ...
        source_labels, source_actual_noise_rate, source_noise_or_not = noise_mnist(args.noise_type,
                                                                                   torch.tensor(source_labels).to(
                                                                                       config["device"]),
                                                                                   args.noise_rate)
        target_labels, target_actual_noise_rate, target_noise_or_not = noise_mnist(args.noise_type,
                                                                                   torch.tensor(target_labels).to(
                                                                                       config["device"]),
                                                                                   args.noise_rate)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noise_type', default='clean', choices=['poison', 'clean', 'pairflip', 'symmetric'])
    ...
    if args.s_dset_file != args.t_dset_file:
        ...
        if config["dataset"] == "office-31":
            ...
            if args.ratio == 1:
                config["ratios_source"] = [1] * 15 + [0] * 16
                config["ratios_target"] = [1] * 15 + [0] * 16
                config["network"]["params"]["class_num"] = 15
            ...
            if args.ratio == 1:
                config["ratios_source"] = [0.5] * 12
                config["ratios_target"] = [0.5] * 12
