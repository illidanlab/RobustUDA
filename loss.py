...
def divergence_loss(source_y, target_y):
    kld = scipy.stats.entropy(source_y, target_y)
    return kld
...
#get the bound of lipchitze continious L
def get_Lbound(features, ad_net, labels):
    atk = torchattacks.PGD(ad_net, eps=8 / 255, alpha=2 / 255, steps=4)
    #atk = torchattacks.FGSM(ad_net, 0.003)
    adv_images = atk(features, labels)
    return adv_images
