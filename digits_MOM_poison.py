...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='MOM robust UDA for digits')
    ...
    parser.add_argument('--iter_type', default='clean', choices=['clean', 'pairflip', 'symmetric'])
   ...
   ##use backdoor attack to poison data
    ...
    source_samples, source_labels = torch.Tensor(source_samples).to(args.device), torch.LongTensor(source_labels).to(args.device)
    ...
    #test_labels_poison = torch.LongTensor(test_labels_poison).to(args.device)
