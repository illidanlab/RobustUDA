...
def build_digits(args, samples, transform):
    if len(samples.shape) < 4:
        channels = 1
    else:
        channels = samples.shape[3]
    sample_new = torch.zeros((samples.shape[0], 3, 28, 28)).to(args.device)
    #if samples.shape[3] == 1:
    #    samples = samples.repeat(1,1,1,3)
    for i in range(samples.shape[0]):
        image = samples[i].cpu().numpy().astype('uint8')
        if channels == 1:
            image = Image.fromarray(image, mode='L')
        elif channels == 3:
            image = Image.fromarray(image, mode='RGB')
        else:
            raise ValueError("{} channel is not allowed.".format(channels))
        sample_new[i] = transform(image)
    return sample_new
