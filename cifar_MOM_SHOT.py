...
def train_target(model_idx, config, dset_loaders, target_dataset_i, model, test_bs, train_bs):
    ...
    for i in range(config["num_iterations"]):
        ...
        if i % config["test_interval"] == 0:
            model.train(False)
            temp_acc, temp_pred = image_classification_test_loaded(test_bs, dset_loaders["test"], model)
            test_poison_acc, pred_test_poison = image_classification_test_loaded(test_bs,
                                                                                 dset_loaders["test_poison"],
                                                                                 model)
            log_str = "  iter: {:05d}, precision: {:.5f}, success_rate: {:.5f}".format(
                i, temp_acc, test_poison_acc)
            config["out_log_file"].write(log_str + "\n")
            config["out_log_file"].flush()
            print(log_str, flush=True)
            if args.cls_par > 0:
                mem_label = obtain_label(target_dataset_i, model, config)
        ...
        inputs_target = inputs_target.to(config["device"])
        features_test, outputs_test = model(inputs_target)


        if args.cls_par > 0:
            if index_t == len_train_target-1:
                pred = mem_label[index_t*train_bs:]
            else:
                pred = mem_label[index_t*train_bs:(index_t+1)*train_bs]
            classifier_loss = nn.CrossEntropyLoss()(outputs_test, pred)
            classifier_loss *= args.cls_par
        else:
            classifier_loss = torch.tensor(0.0).to(config["device"])
    ...
    model.train(False)
    temp_acc, pred_test = image_classification_test_loaded(test_bs, dset_loaders["test"], model)
    test_poison_acc, pred_test_poison = image_classification_test_loaded(test_bs,
                                                                         dset_loaders["test_poison"],
                                                                         model)
    log_str = "Finish SHOT!  precision: {:.5f}, success_rate: {:.5f}".format( temp_acc, test_poison_acc)
    config["out_log_file"].write(log_str + "\n")
    config["out_log_file"].flush()
    print(log_str, flush=True)
    model.train(True)
    ...
    return pred_test, pred_test_poison
...
def obtain_label(dset, model, config):
    start_test = True
    with torch.no_grad():
        iter_test = iter(dset)
    ...
    accuracy = torch.sum(torch.squeeze(predict).float().to(config["device"]) == all_label.to(config["device"])).item() / float(
        all_label.size()[0])
...
def train(config, source_samples, source_labels, target_samples, target_labels, test_samples, test_labels):
    ...
    for model_idx in range(args.block):
        ...
        pred_test, pred_test_poison = train_target(model_idx, config, dset_loaders, target_dataset_i, base_network, test_bs, train_bs)
        ...
        if args.corrupt != 'clean':
            ...
            vote(test_bs, pred_test_poison_all, dset_loaders["test_poison"], config["out_log_file"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MOM robust UDA for cifar with fine-tune')
