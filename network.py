...
class LeNet(nn.Module):
    def __init__(self, ma=0.0, input_channel=1):
        super(LeNet, self).__init__()
        self.conv_params = nn.Sequential(
                nn.Conv2d(input_channel, 20, kernel_size=5),
                nn.MaxPool2d(2),
                nn.ReLU(),
...
class Model_digit(nn.Module):
    def __init__(self, num_classes=10, width_mult_list=None):
        super(Model_digit, self).__init__()
        self.features = []
        self.slim_ratio = 1.0
    ...
    def set_slim_ratio(self, ratio):
        if self.slim_ratio != ratio:
            self.slim_ratio = ratio
            for m in list(self.modules()) + [self]:
                if any([
                    ...
                    assert ratio in self.width_mult_list
                    assert hasattr(m, 'width_mult'), f"model {m} does not have width_mult."
                    m.width_mult = ratio