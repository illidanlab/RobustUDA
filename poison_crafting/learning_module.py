...
def find_indices(train_label, dataset_size, poison_indices, poisoned_label, poison_instances):
    good_idx = np.array([])
    batch_tar = np.array(train_label)
    num_classes = len(set(batch_tar))
    num_per_class = int(dataset_size / num_classes)
    for label in range(num_classes):
        ...
        if label == poisoned_label and len(poison_instances) > 0:
    ...
    clean_indices = good_idx.astype(int)
    return clean_indices

def get_Poison_dataset(train_sample, train_label, poison_instances, size=None, transform=None, poison_indices=None):
    poison_indices = np.array([]) if poison_indices is None else poison_indices
    dataset_size = size if size is not None else train_sample.shape[0]
    poisoned_label = (
        None if len(poison_instances) == 0 else poison_instances[0][1]
    )
    num_clean_samples = dataset_size - len(poison_instances)
    clean_indices = find_indices(train_label, dataset_size, poison_indices, poisoned_label, poison_instances)
    indicate = np.zeros((dataset_size))
    for index in range(dataset_size):
        if index > num_clean_samples - 1:
            train_sample[index], train_label[index] = poison_instances[index - num_clean_samples]
            indicate[index] = 1 #output is 1 for poison
        else:
            new_index = clean_indices[index]
            train_sample[index], train_label[index] = train_sample[new_index], train_label[new_index]
            indicate[index] = 0   # last output is 0 for clean
    return train_sample, train_label, indicate
...
class PoisonedDataset(data.Dataset):
    ...
    def find_indices(self):
        ...
        for label in range(num_classes):
            all_idx_for_this_class = np.where(batch_tar == label)[0]
            num_per_class = len(all_idx_for_this_class)
            all_idx_for_this_class = np.setdiff1d(
...
def get_model(model, dataset="CIFAR10"):
    ...
    if dataset == "cifar10":
        if model == "resnet18":
            ...
            net = models.resnet32(num_classes=9)
        elif model == "mobilenet_v2":
            net = models.MobileNetV2(num_classes=9)
        elif model == "alexnet":
            net = models.AlexNet(num_classes=9)
        elif model == "htbd_alexnet":
            net = models.HTBDAlexNet(num_classes=9)
        elif model == "vgg11":
            net = models.vgg11(num_classes=9)
        else:
            print(
...
def compute_perturbation_norms(poisons, dataset, base_indices):
    ...
    """
    perturbation_norms = []
    poison_tensors = [img for img, label in poisons]
